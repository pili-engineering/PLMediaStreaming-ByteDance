...
- (void)initProcessor:(CGSize)videoSize {
    ...
    [EAGLContext setCurrentContext:context];
    _processor = [[BEFrameProcessor alloc] initWithContext:context videoSize:videoSize];
}

- (CVPixelBufferRef)process:(CVPixelBufferRef)pixelBuffer timeStamp:(double)timeStamp {
    BEProcessResult *result = [_processor process:pixelBuffer timeStamp:timeStamp];

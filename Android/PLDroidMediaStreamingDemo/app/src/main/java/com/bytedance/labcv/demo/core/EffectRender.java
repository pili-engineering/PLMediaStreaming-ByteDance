...
public class EffectRender {
    ...
    public void drawFrameOnScreen(int textureId,BytedEffectConstants.TextureFormat srcTetxureFormat,int textureWidth, int textureHeight,  int rotation, boolean flipHorizontal, boolean flipVertical) {
        ...
        if (rotation % 180 == 90){
            mMVPMatrix = GlUtil.changeMVPMatrixCrop(mMVPMatrix,textureHeight, textureWidth, mSurfaceWidth, mSurfaceHeight);

        }else {
            mMVPMatrix = GlUtil.changeMVPMatrixCrop(mMVPMatrix, textureWidth, textureHeight, mSurfaceWidth, mSurfaceHeight);
